...
class GptOss_20bClient(AgentInterface):
    """
    Client implementation for GPT OSS 20B.
    ...
    Attributes:
        name (str): Hardcoded as "GPT OSS 20B".
        model (str): Specific model path: "openai/gpt-oss-20b".
    """

    def __init__(self, api_key: str | None = None):
        """
        Initializes the GPT OSS 20B Client with the specific Groq model name.

        Args:
            api_key (str, optional): Groq API key. If None, attempts to inherit 
                from environment variables managed by the FailoverManager.
        """
        super().__init__(name="GPT OSS 20B", model="openai/gpt-oss-20b", api_key=api_key)

    @handle_agent_errors(AgentAPIError)
    def generate_text(self, prompt: str) -> str:
        """
        Generates text using the GPT OSS 20B model.
